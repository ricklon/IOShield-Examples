...
//*     LED row is refreshed to be inverted from current status
//*
//*
...
int  mySw1State;
  ...
  if (mySw1State != gSwitch1State)
  {
  int  ii;
  
   gSwitch1State  =  mySw1State;
   for(ii; ii < kNUMLEDS; ii++) 
    {
    digitalWrite(LEDPins[ii], gSwitch1State);
    }
    gCurrentPin = -1;
  }
  ...
  {
    digitalWrite(LEDPins[gCurrentPin], gSwitch1State);   
    digitalWrite(LEDPins[newPin], !gSwitch1State);   
    gCurrentPin  =   newPin;
